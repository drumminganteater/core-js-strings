...
function isPalindrome(str) {
  const lowered = str.toLowerCase().replace(/[\W_]/g, '');
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (str.charAt(i) === str.charAt(i).toUpperCase())
      result += str.charAt(i).toLowerCase(value);
    else result += str.charAt(i).toUpperCase(value);
  }
  return result;
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-z]/gi, (i) => cipher[letters.indexOf(i)]);
}
